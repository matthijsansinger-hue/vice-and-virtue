...
- **Roles** — **two tier matrices** (`RolesSection`, account-gated): rows S→D, each with a vertical tier indicator + translucent tier-tinted background, split **Vice | Virtue** by a centre divider with full **role-card art** (`/cards/<role>.png`) + camp-coloured borders (red vice / blue virtue). The **first matrix = roles you own**; **below it, a second matrix = roles you haven't unlocked** — each greyed (lock icon + name + "1000 LP"). **Desktop hover** shows an owned role's description on the card; **mobile tap** opens a readable popup. **Tapping a locked role** (desktop or mobile) opens an **unlock modal** (card + description + your LP balance + "Unlock for 1000 LP", disabled when short) → `unlockRoleWithLe` → on success refreshes `econ` so the role jumps to the owned matrix. Shared `matrix(roles, renderCard)` helper renders both, skipping empty tiers (the locked set is S/A/B/C only). The hub passes `econ` + an `onUnlocked` refresh callback.
...
src/components/
  ...
  RoleOverview.tsx               # `role_overview` phase: this game's full cast (role_pool) split Vice | Virtue with RoleIcons + ability lines; majority-continue → lore_intro
