...
- **Day cycle — three named phases (Reflection / Action / Consultation), each spanning sub-phases:**
  ...
  - **Action** — outreach (120s one-on-one chats; **mandatory**, imprisoned players ARE eligible, DM history resets each day) → store (60s individual potion shopping + Fanaticism check/detonate + Sacrifice, `store` phase, migration 058 — see "Store & potions") → **store_summary** (anonymous recap of the shop's deaths — combat potions, bomb detonations, sacrifices — shown before the camp abilities, migration 072) → group_action (60s: two simultaneous camp/"team" abilities, see below). The **whole Action phase shares the outreach background**.
...
- **Badges (`lib/badges.ts`):** an 83-badge catalog across five tiers (Divine → Noble → Primal → Verdant → Earthen), incl. a 60-badge per-role win matrix (1/5/15/40/100 wins → one threshold per tier) and totals/specials. **Medallions (`BadgesShowcase.tsx` `Medallion`, now exported) use painted per-tier frame art** (`public/badge-frame-<tier>.png`, AI art with the black background edge-flood-filled to transparency) — the old procedural SVG `Decor` is gone. Divine/Noble/Primal frames are **punched** (transparent centre): the icon is drawn *behind* the frame so the ring + cardinal gems sit in front, with each gem's gold setting preserved; Verdant/Earthen keep a solid frame with the icon on top. A per-tier ellipse `FRAME_WINDOW` + a `PUNCHED` flag drive the layout. **Role badges** show that role's tier-tinted character icon (`public/badge-icons/<role>-<tier>.png`, 12 roles × 5 tiers — a grey head colorized to the tier hue, on a dark recessed disc); **glyph badges** (no character art) show an inline icon on a per-tier `GLYPH_COIN` coin — dark+gold for most tiers, **white+gold (Divine)** and **purple+gold (Noble)** to match those bright frames. An optional `BadgeDef.glyphText` renders short text instead of an icon (the **Founder** badge shows "19" — it now triggers for the first **19** accounts, not 95, and its hover/popup shows the viewer's spot "n/19" via a `founderRank` threaded from `/profile`). Most badges derive live from `game_results` + account age; event/claim badges are keys in `user_achievements`. Discord badge auto-awards on clicking the home Discord link.
  - **Visibility model (reworked):** locked badges are no longer shown in the collection grid. `badgeCategory()` (in `badges.ts`) sorts every badge into: **character** (per-role win badges → shown under "Wins per character" with progress, NOT in the grid), **milestone** (total-wins + total-games → shown in a "Milestones" section with progress, NOT in the grid), **goal** (account/Founder/friend/Discord/minigame — stay visible even when locked), and **secret** (role-ability event badges — hidden until earned). The `BadgesShowcase` grid therefore shows only goals + earned secrets, grouped by **collapsible tiers (collapsed by default)**; tier headers show an earned count only (never a total, so secret counts don't leak). All five tier headers always appear — Verdant/Primal hold only secret badges, so they show a "nothing here yet" hint until one is unlocked. Progress helpers: `roleBadgeProgress(roleId, wins)` + `milestoneProgress(kind, value)` return `{earned, next, current, target}`; `ProfileStats` renders earned medallions + the next dimmed target with `current/target` (badges beyond the next are not shown). The FeaturedBadges picker is unchanged — any earned badge (incl. character/milestone) is still featurable.
...
- **Soul Fragments** (loot box; display name — renamed from "Soul Shards", internal names like `unopened_shards`/`open_soul_shard` stay) — granted by **daily login** and **each of your first three wins of the day** (migration 077). Opening one gives a guaranteed **+50 XP**, then rolls **0.1% instant role unlock / 9% Mano (+10) / ~90.9% LP (+10)**. (The stated 91/9/0.1 split sums to 100.1%, so LP absorbs the rounding.) RNG is server-side. The **hub HUD daily-reward icon** is gold-outlined while anything is still claimable and greys out once both the login shard and all 3 win shards are banked.
...
- **Roles** — **two tier matrices** (`RolesSection`, account-gated): rows S→D, each with a vertical tier indicator + translucent tier-tinted background, split **Vice | Virtue** by a centre divider with full **role-card art** (`/cards/<role>.png`) + camp-coloured borders (red vice / blue virtue). The **first matrix = roles you own**; **below it, a second matrix = roles you haven't unlocked** — each greyed (lock icon + name + its **tier price**, `roleUnlockCost(tier)`). **Desktop hover** shows an owned role's description on the card; **mobile tap** opens a readable popup. **Tapping a locked role** (desktop or mobile) opens an **unlock modal** (card + description + your LP balance + "Unlock for N LP" at that role's tier price, disabled when short) → `unlockRoleWithLe` → on success refreshes `econ` so the role jumps to the owned matrix. Shared `matrix(roles, renderCard)` helper renders both, skipping empty tiers (the locked set is S/A/B/C only). The hub passes `econ` + an `onUnlocked` refresh callback.
...
src/components/
  ...
  Lobby.tsx                      # create-room screen + Public/Private toggle + Role-assignment toggle (Choose | Random; Random adds a "Configure roles" button → RoleConfigModal) + kick/leave (host too → hands off to second-joined via leaveRoom)/block/report + account avatars + featured badges + InviteToGame button (header). Desktop two-column (players | host controls).
  ...
  ProfileStats.tsx               # shared stats display: summary + Milestones progress (total wins/games) + Wins-per-character with each character's badge progress + recent games
  BadgesShowcase.tsx             # painted-frame tier badge medallions (punched frames + tinted character icons + per-tier glyph coins); hover bubble / tap popup; collection grid shows goals + earned secrets only, collapsible tiers (collapsed by default); exports BadgesShowcase + BadgeTile + Medallion
  ...
  RoleOverview.tsx               # `role_overview` phase: this game's full cast (role_pool) split Vice | Virtue; each role is tap-to-open (RoleInfoModal = camp/tier/cost + full description), no inline ability line so long abilities fit; majority-continue → lore_intro
