...
- **Gambling kills via the normal queue** — `gambling_roll` rolls server-side and, on a match, writes `pending_action='kill'` so the existing kill resolution (protect + extra lives) handles it; no special path. **Pride** uses `rooms.pride_target` read by `submit_minigame_guesses` to zero that player's round, and a `player_notices` reveal. **Diligence** is immune to the wrong-tag zero inside `submit_minigame_guesses`, which also stores `minigame_correct` for the result-screen paid count.
