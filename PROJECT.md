...
- **Wrath/Love conversion is instant role-replacement, not a queued action (migration 067)** — `convert_player` rewrites the target's `player_secrets.role` to `vice_worshipper` (Wrath) / `virtue_seeker` (Love) the moment it's cast, so the camp switch is live for the win check + minigame scoring with no `resolve_role_action` change. It cancels the target's queued action and marks them `acted_this_day` (they can't act twice the turn they flip). **It's a gamble** (200 SE, migration 070) — it lands only on a **non-S-tier** role of the wanted camp (so only a Virtue can be corrupted / only a Vice turned, and **all S-tier roles — Murder, Wrath, Empathy, Love — are immune**, which also makes Wrath and Love immune to each other), but the 200 SE is spent on a whiff either way. The whiff message is camp/tier-agnostic ("resisted your influence") so it doesn't leak whether the target was the wrong camp or an immune S-tier. Wrath's converts set `follower_of` (binding them to that Wrath); Love's clear it. **`relinquish_follower`** consumes a follower (instant death, bypassing their own extra lives like a Sacrifice does) to mint a Wrath **extra life** (the 066 buffer) — so Wrath turns Virtues into a renewable life supply. **Love's tie-break** (`love_tiebreak`, one day) only breaks a tie Love actually **voted into** — it biases a deadlock toward her pick rather than being a free imprisonment; it's mirrored in `consultation_tally` so the host's re-vote gate and the result screen agree with `resolve_consultation`. `love_tiebreak` stays server-only (sending it would out Love).
