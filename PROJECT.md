...
- **Pre-game flow (once at start):** depends on `rooms.role_assign_mode` (migration 063):
  - **'choose' (ranked; lobby slider for casual lands in the next batch):** match forms → **role_select** (30s: each player is dealt a camp + a tier and picks their role within it; a click is a tentative pick camp-mates see **anonymously by tier** in a team panel — desktop left, mobile bottom — then "Lock in"; on expiry the host resolves, stragglers get their tentative pick else a random role of their tier) → **role_overview** (this game's full cast, Vice | Virtue, majority-continue) → lore_intro → **straight into the first role_action** (no role_reveal — you picked your own card).
  - **'random' (default, the current casual flow):** lobby → game_overview → lore_intro (3.5s zoom + 0.5s fade-to-black) → role_reveal → first role_action.
