...
- **Role ownership** — `account_role_unlocks`. **The 12 original roles are unlocked by default** (`DEFAULT_UNLOCKED_ROLES`); the **8 new roles (migration 062: Wrath/Love/Gambling/Determination/Fanaticism/Generosity/Pride/Diligence)** are LOCKED and cost **1000 LP** each via `unlock_role` (or the rare 0.1% shard drop). The new roles are **collection-only for now** — shown in the Roles tab as unlockable, but their gameplay isn't built, so `isPlayableRole()` (roles.ts, an exclusion set) keeps them out of match assignment, the badge catalog, wins-per-character, the ranked loadout, and the in-game rules. `c_all_roles` in both economy RPCs (`unlock_role`/`open_soul_shard`) was extended to the 20; `c_default` stays the 12.
...
src/components/
  ...
  RoleConfigModal.tsx            # host random-mode config: per camp per tier, pick the slot's role (multi-option tiers get buttons + "Random"; single-option tiers fixed; D = filler) → rooms.role_config
