...
- **Role ownership** — `account_role_unlocks`. **The 12 original roles are unlocked by default** (`DEFAULT_UNLOCKED_ROLES`); the **8 new roles (migration 062: Wrath/Love/Gambling/Determination/Fanaticism/Generosity/Pride/Diligence)** are LOCKED and cost **1000 LP** each via `unlock_role` (or the rare 0.1% shard drop). **Once unlocked they're pickable in role select** (migration 065: `select_role` enforces ownership server-side; `vv_role_camp`/`vv_role_tier` know all 20, so camps/win checks/minigame scoring treat them correctly; guests own only the 12). Locked roles show **greyed + lock** on the role-select screen. **5 of the 8 abilities are now built** (migration 066, batch 1): **Determination, Generosity, Gambling, Pride, Diligence** + the shared **extra-lives** system. The remaining 3 (**Wrath, Love** = followers; **Fanaticism** = bombs) still fall through `RoleAction`'s `IMPLEMENTED_ABILITIES` "not implemented yet" panel and play as plain camp members. `isPlayableRole()` (roles.ts) still only filters the META surfaces (badge matrix, wins-per-character, rules list, random-mode host config, orphaned loadout) — all 8 are playable. `RoleIcon` falls back to the card art for roles without a `role-icons/*.png` head icon. `c_all_roles` in both economy RPCs was extended to the 20; `c_default` stays the 12.
...
- **Extra lives (migration 066)** — `player_secrets.extra_lives` is a stored buffer (Determination buys 100 SE/stackable, Generosity grants for 200, later Wrath earns by relinquishing a follower). In `resolve_role_action`, AFTER all kills/hospitalisations are tallied but BEFORE kill-counting/achievements/win check, each player in the dead set with an extra life is pulled out (−1 life) — so an absorbed kill counts as if it never happened; then the same for the hospital set. Consultation **instant-Sacrifice bypasses extra lives** (it bypasses Justice protect too — a mutual-death pact shouldn't be cheated by a stored life). The owner sees their count via `get_my_secrets` → a ♥N chip in the TopBar.
