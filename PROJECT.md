...
- **Role ownership** — `account_role_unlocks`. **The 12 original roles are unlocked by default** (`DEFAULT_UNLOCKED_ROLES`); the **8 new roles (migration 062: Wrath/Love/Gambling/Determination/Fanaticism/Generosity/Pride/Diligence)** are LOCKED and cost **1000 LP** each via `unlock_role` (or the rare 0.1% shard drop). **Once unlocked they're pickable in role select** (migration 065: `select_role` enforces ownership server-side; `vv_role_camp`/`vv_role_tier` know all 20, so camps/win checks/minigame scoring treat them correctly; guests own only the 12). Locked roles show **greyed + lock** on the role-select screen. **7 of the 8 abilities are now built**: **Determination, Generosity, Gambling, Pride, Diligence** + the shared **extra-lives** system (migration 066, batch 1), and **Wrath, Love** (migration 067, batch 2 — conversion/followers + Love's consultation tie-break). The remaining 1 (**Fanaticism** = bombs) still falls through `RoleAction`'s `IMPLEMENTED_ABILITIES` "not implemented yet" panel and plays as a plain camp member. `isPlayableRole()` (roles.ts) still only filters the META surfaces (badge matrix, wins-per-character, rules list, random-mode host config, orphaned loadout) — all 8 are playable. `RoleIcon` falls back to the card art for roles without a `role-icons/*.png` head icon. `c_all_roles` in both economy RPCs was extended to the 20; `c_default` stays the 12.
