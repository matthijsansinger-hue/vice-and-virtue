-- ============================================
-- Combat potions: kill / hospitalise / protection (migration 059) — store 2b.
-- ============================================
-- The three combat potions resolve in the NEXT day's reflection, reusing the
-- existing kill -> protect -> hospitalise machinery in resolve_role_action:
--   * Kill potion (300 SE): a live buyer kills a target (protection blocks).
--   * Hospitalise potion (200 SE): hospitalise a target 1 day (protection blocks).
--   * Protection potion (200 SE): the buyer shields THEMSELVES next reflection
--     (blocks Murder, Intoxication, and incoming potion kills/hospitalise).
-- "One of each type per day": re-buying a type you already armed is rejected.
-- The buyer must still be alive at resolution for their potion to fire; the
-- combat potion fields are cleared each reflection (they fired).
--
-- Extends buy_potion (adds the three combat branches) and resolve_role_action
-- (folds the potions into kill/protect/hospital). Bases resolve_role_action on
-- the live migration-056 version + the potion bits, and mirrors it into
-- db/schema.sql (which still held the pre-056 version).

-- ---- buy_potion: add kill / hospitalise / protect ------------------------
...
-- ---- resolve_role_action: fold in the combat potions ---------------------
-- (Migration-056 version + Protection self-shield, Kill potion, Hospitalise
-- potion, and clearing the combat potion fields after they fire.)
