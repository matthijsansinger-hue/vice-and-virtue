...
-- ============================================
-- New account-level progression, kept in tables SEPARATE from `profiles` so
-- the client can't edit it directly: profiles has a "users update their own
-- profile" policy (fine for username/avatar/featured_badges), but spendable
-- currencies must only ever change through the SECURITY DEFINER RPCs below.
-- account_economy/account_role_unlocks therefore have NO client insert/update
-- policy — only the RPCs (which run as definer) write them.
--
-- Naming: the in-MATCH ability resource is "Soul Energy" (players.soul_energy,
-- reset every game). THIS account currency is "Life Experience" (LE) — earned
-- from Soul Shards + matches, spent to unlock roles. Different thing.

-- Safe re-run: these tables hold not-yet-live progression data, so drop any
-- earlier version before recreating.
drop table if exists account_match_rewards cascade;
drop table if exists account_role_unlocks cascade;
drop table if exists account_economy cascade;
...
-- Roles an account has unlocked beyond the default starter set (economy.ts
-- DEFAULT_UNLOCKED_ROLES — currently all 12, so this stays empty until new
-- roles ship). Only roles unlocked beyond the default get a row.
...
-- Per-room match-reward ledger so match XP / LE / first-win shards are granted
-- at most once per account per game, even if game_over re-triggers.
...
--   0.1%  -> instantly unlock a random still-locked role (none yet -> LE)
--   9%    -> Mano
--   ~90.9% -> LE   (the stated 91/9/0.1 sums to 100.1%; LE absorbs the 0.1%
...
declare
  ...
  v_role text := null;
  v_locked text[];
  -- mirror roles.ts (ROLES) + economy.ts DEFAULT_UNLOCKED_ROLES (all 12 now)
...
-- ---- Spend LE to unlock a role (roles collection / shop) ------------------
...
-- ---- Host-side: per-match XP + LE + first-win shard for account players ---
