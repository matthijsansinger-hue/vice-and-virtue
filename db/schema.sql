...
-- ============================================
-- Two-sided queue: join as Vice/Virtue; ranked_matchmake() forms a balanced
-- game (N+N) when both sides have enough waiting, creates a ranked room, seats
-- everyone honoring their chosen side, assigns roles, and auto-starts. Read-own
-- RLS so nobody can see who queued which side (that would leak camps).
...
-- Forms a match if one is possible (advisory-locked so it runs once at a time);
-- returns the new room code or null.
create or replace function ranked_matchmake()
returns text language plpgsql security definer set search_path = public as $$
declare
  c_min constant int := 3;
  c_max constant int := 10;
  v_vice int; v_virtue int; v_n int;
...
begin
  ...
  into v_vice, v_virtue from ranked_queue where status = 'waiting';

  v_n := least(v_vice, v_virtue, c_max);
  if v_n < c_min then return null; end if;

  select array_agg(user_id order by joined_at) into v_vice_users
  from (select user_id, joined_at from ranked_queue
        where status = 'waiting' and side = 'vice' order by joined_at limit v_n) q;
  select array_agg(user_id order by joined_at) into v_virtue_users
  from (select user_id, joined_at from ranked_queue
        where status = 'waiting' and side = 'virtue' order by joined_at limit v_n) q;
