...
// The handoff defined the body as a `window.PUNCH_HOSPITAL` sibling-script global and
// `draw` just invoked it; here that exact function body is inlined as a draw-local
// (the `window.` prefix dropped) so the module is self-contained. Drawing code verbatim.
...
const clip: ClipConfig = {
  ...
  fadeFromBlack: true,
  draw(c, t, AB) {
    function PUNCH_HOSPITAL(c,t,AB,opts){
  ...
  // attacker (left, faces right) + punching arm
  ...
  // victim (right): recoils, then collapses onto the cot
  if(collapse<0.55){
    ...
    figure(c,{x:Vx,base:sinkBase,s:0.95,fill:FIG});
    c.restore();
  }
  if(collapse>0.35){
    const ly=FLOORY-150;
  ...
  // impact starburst on the punch landing
  ...
  // hospital cross glowing above the cot
  if(settle>0.05){
    const cy2=FLOORY-300, ccx=cotX;
    const gl=interp([1.45,1.8],[0,1],E.easeOutBack)(t);
