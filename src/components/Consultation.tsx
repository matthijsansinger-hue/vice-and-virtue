...
// Computes the vote tally from the given players (used for display).
...
export function Consultation({
  ...
  const voters = nonImprisoned; // only non-imprisoned players vote
  const votableTargets = nonImprisoned.filter((p) => p.id !== myPlayer?.id);

  const votedCount = voters.filter((p) => p.vote).length;
  const allVoted = voters.length > 0 && voters.every((p) => p.vote);

  // Safety guard: not enough free players to keep playing.
  if (nonImprisoned.length <= 1) {
    return (
      <Centered className="bg-consultation-bg text-cream">
        ...
        <p className="mt-2 max-w-sm text-cream/70">
          Win conditions aren&rsquo;t built yet. Only {nonImprisoned.length}{" "}
          player(s) free.
        </p>
  ...
  // Imprisoned player: passive screen, no vote.
  ...
  // Voting UI: this player hasn't voted yet, and not everyone is in.
  if (myPlayer && !myPlayer.vote && !allVoted) {
    return (
  ...
  // All voted: show result + (host) advance to the next day.
